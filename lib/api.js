...
export const getCompareMessagesByWeek = (accountId, apiDataCompanyIds, customerGroupId, week, year) =>
    fetch(`http://192.168.0.215:5000/data-compare?sid=${accountId}&company_id=[${apiDataCompanyIds}]&customer_group_id=${customerGroupId}&week=${week}&year=${year}`)
...
export const getCompareMessagesByDateRange = (accountId, apiDataCompanyIds, customerGroupId, startDate, endDate) =>
    fetch(`http://192.168.0.215:5000/data-compare?sid=${accountId}&company_id=[${apiDataCompanyIds}]&customer_group_id=${customerGroupId}&start_date=${startDate}&end_date=${endDate}`)
    // fetch(`${config.apiUrl}/data-compare?sid=${accountId}&company_id=[${apiDataCompanyIds}]&customer_group_id=${customerGroupId}&week=${week}&year=${year}`)
    .then((res) => {
        if (res.ok) {
            return res.text();
